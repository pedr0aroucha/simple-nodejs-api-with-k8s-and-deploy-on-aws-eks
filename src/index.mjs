...
function createHandler(algorithm) {
  return async (req, res) => {
    ...
    if (algorithm === "aes") {
      ...
      if (key.length !== 32)
        return res.status(400).json({ error: "Key should be 32 chars long" });
      if (!!decrypt && !iv)
        return res.status(400).json({ error: "IV is missing" });
      if (!!decrypt && iv)
        return res.status(200).json({ response: aesDecrypt(key, iv, data) });
